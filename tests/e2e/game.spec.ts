...
test('arithmetic tower defence MVP is playable in the browser', async ({ page }) => {
    ...
    await expect(page.getByTestId('answer-popup-close')).toHaveText('Skip (1 left)');
    await page.getByTestId('answer-popup-close').click();
    await expect(page.getByTestId('build-popup')).toBeHidden();
    await expect.poll(() => page.evaluate(() => window.arithmeticAnnihilation!.getAvailableSkips())).toBe(0);
    await clickWorldPoint(page, buildable!.worldX, buildable!.worldY);
    await expect(page.getByTestId('build-popup')).toBeVisible();
    await expect(page.getByTestId('answer-popup-close')).toBeDisabled();
    await expect(page.getByTestId('answer-popup-close')).toHaveText('No skips left');
    await expect(page.getByTestId('pause-overlay')).toBeHidden();
    ...
    await expect(page.getByTestId('incorrect-answer-wait')).toHaveCount(0);
    await expect(answerInput).toBeVisible();
    ...
    await expect(page.getByTestId('incorrect-answer-wait')).toContainText(/Wait [1-6] seconds? to continue\./);
    ...
    await expect(reviewInput).toBeEnabled({ timeout: 7_000 });
    await expect(reviewInput).toBeFocused();
    await expect(page.getByTestId('incorrect-answer-wait')).toHaveText('Type the correct answer to continue.');
...
test('mobile answer flow keeps choices and uses an in-game correction number pad', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="answer-number-key"]').first()).toBeDisabled();
    ...
    await expect(page.locator('[data-testid="answer-number-key"]').first()).toBeEnabled({ timeout: 7_000 });
