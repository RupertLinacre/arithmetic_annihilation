...
The start screen offers the original one-player game and a two-player game. Two-player mode can be played against a local computer opponent or against a friend using a four-character invite code. Each player builds on one half of the original 24×14 arena and protects a separate base. Blue and red team artwork makes each player's base, monsters, and towers immediately distinguishable.
